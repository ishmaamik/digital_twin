"""
Rehearsal-protocol Stage-2 study: k-NN, Random Forest, and Fourier-encoded
k-NN, extended to the 6 lane-width / site comparisons (Scenarios 1, 3, 4,
7, 32straight, 33straight). Fourier-RF and a TinyMLP variant are out of
scope for this study by request (same scope as the earlier
train_baselines_rehearsal.py study).

This script is fully self-contained (its own COMPARISONS/SWEEP_POINTS/
N_SEEDS), matching the precedent set by rehearsal_study/train_baselines_rehearsal.py
(the earlier Scenario-1-4-only rehearsal study, whose COMPARISONS/results
this script does not touch).

REPLACES the old target-only-fit protocol (see the module docstring of
train_baselines_rehearsal.py for the full history): under that protocol,
each classifier at every sweep point N > 0 was fit from scratch on ONLY
the N target samples, throwing away the source scenario entirely once
N > 0. This was confirmed directly: two comparisons sharing the same
target scenario produced byte-identical non-zero-sample results (diffing
stage1_time_of_day_at_ruralroad_*.mat against stage1_site_during_night_*.mat,
both targeting Scenario 4), since target-only fits have no way for the
source scenario to influence anything past the zero-shot point.

REHEARSAL protocol, per seed:
...
consistently across both algorithms here (same scheme as
train_baselines_rehearsal.py):
...
These are all cheap, near-zero-training-cost classifiers (no gradient
descent, no epochs) -- a full run across N_SEEDS=10 seeds, 6 comparisons,
and 23 sweep points per (seed, comparison) is expected to finish in
minutes, not hours. Check for "Rehearsal baseline comparison complete" in
the log, not just file existence, before treating a run as finished.

Must be run from the repository root, e.g.:
    python rehearsal_study/train_baselines_cross_scenario_rehearsal.py --model knn
    python rehearsal_study/train_baselines_cross_scenario_rehearsal.py --model rf
    python rehearsal_study/train_baselines_cross_scenario_rehearsal.py --model fourier_knn
"""
import argparse
import json
import os
...
# The 6 lane-width / site comparisons this Stage-2 rehearsal study covers.
# Scenario 32straight/33straight (not the raw 32/33) are used for the
# narrow-site targets -- see python/preprocess_college_ave_straight.py for
# why (Scenarios 32/33's road bends partway through the captured segment,
# which was found to destabilize recovery curves; the *straight variants
# exclude the bend).
COMPARISONS = [
...
def main():
    ...
    print(f"Running Stage 2 REHEARSAL baseline with model={args.model}", flush=True)
    ...
    header = "real_samples".rjust(12) + "".join(f"{lbl:>22}" for lbl in summary)
    print(header, flush=True)
    for i, p in enumerate(points):
        row = str(p).rjust(12) + "".join(f"{summary[lbl]['top2_accuracy'][i] * 100:>21.2f}%" for lbl in summary)
        print(row, flush=True)

    # "_stage2" distinguishes this from train_baselines_rehearsal.py's own
    # stage1_summary_{model}_rehearsal.json (the earlier Scenario-1-4-only
    # study) -- both scripts previously wrote the SAME path, so whichever
    # ran last silently truncated the other's summary down to its own
    # comparisons. Kept as two separate files rather than merged so each
    # study's summary is independently reproducible by rerunning its own
    # script, with no risk to the other's.
    summary_path = os.path.join(RESULT_DIR, f"stage1_summary_{args.model}_rehearsal_stage2.json")
