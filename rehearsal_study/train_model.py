...
model_name added so this loop can be reused across architectures via
MODEL_REGISTRY (see model.py), instead of hard-coding FullyConnected.
"""
import torch
import numpy as np
...
def train_model(
    ...
    net = MODEL_REGISTRY[model_name](num_classes)
    num_params = sum(p.numel() for p in net.parameters())
    print(f"model={net.name} params={num_params}", flush=True)
    if model_path:
