"""
Rehearsal-protocol Stage-2 study, batch 3: k-NN, Random Forest, and their
Fourier-encoded variants, covering 8 NEW comparisons anchored on Scenario 7
(wide site) and Scenario 32straight (narrow site) as SOURCE:

    7 -> 2, 7 -> 3, 7 -> 4, 7 -> 32straight,
    32straight -> 1, 32straight -> 2, 32straight -> 3, 32straight -> 33straight

Kept as its own script (own COMPARISONS/output files) rather than folded
into train_baselines_cross_scenario_rehearsal.py's 6-comparison batch, so
neither batch's results or summary.json can collide with or overwrite the
other's. train_baselines_cross_scenario_rehearsal.py already established
that its summary.json needed a distinct name from
train_baselines_rehearsal.py's (the original Scenario-1-4-only study) for
exactly this reason; this script continues that numbering
("_stage3") rather than reintroducing the same collision a third time.

REHEARSAL protocol (identical to the sibling stage-2 scripts -- see
train_baselines_cross_scenario_rehearsal.py's docstring for the full
history/justification of why target-only fitting was replaced), per seed:
...
the sibling scripts):
  - Random Forest / Fourier-RF: sample_weight gives every source row
    weight 1/n_source and every target row weight 1/N, so the two pools
    sum to equal total weight (1.0 each) in the fit regardless of how
    small N is.
...
NOTE on Scenario 32straight/33straight data quality: the raw
data/scenario32straight_real_beam_pwr.mat and
scenario33straight_real_beam_pwr.mat files each contain a small number of
NaN cells (confirmed pre-existing, present in already-committed
non-rehearsal results too -- not introduced by any rehearsal script). This
protocol runs unmodified against that data with no special-casing, filtering,
or NaN-guarding, per explicit instruction. When 32straight or 33straight is
the TARGET (7->32straight, 32straight->33straight here), a small number of
seeds' "pwr" (relative power) metric may come out NaN for rows whose held-out
test split happens to include one of those NaN cells; "acc" is unaffected
since it does not divide. This is expected and is a data-source property, not
a run failure -- do not treat it as a reason to alter this script's behavior.

COMPUTE WARNING: every one of the 23 sweep points (22 SWEEP_POINTS + the
zero-shot point) fits on n_source (+ oversampled to 2*n_source for
knn/fourier_knn) rows instead of just N. Across N_SEEDS=10 seeds, 8
comparisons, and 4 models this is cheap (minutes, not hours) but still
proportionally heavier than the old target-only sweep. Check for
"Rehearsal baseline comparison complete" in the log, not just file
existence, before treating a run as finished.

Must be run from the repository root, e.g.:
    python rehearsal_study/train_baselines_cross_scenario_rehearsal_stage3.py --model knn
    python rehearsal_study/train_baselines_cross_scenario_rehearsal_stage3.py --model rf
    python rehearsal_study/train_baselines_cross_scenario_rehearsal_stage3.py --model fourier_knn
    python rehearsal_study/train_baselines_cross_scenario_rehearsal_stage3.py --model fourier_rf
"""
import argparse
import json
import os
...
# The 8 comparisons this batch covers: Scenario 7 (wide site) and Scenario
# 32straight (narrow site) each used as SOURCE against several targets,
# including one another's usual target-side partner (32straight -> 33straight).
COMPARISONS = [
...
MODEL_CHOICES = ["knn", "rf", "fourier_knn", "fourier_rf"]


def load_global_normalization():
    # Pooled across all 7 scenarios (1-4, 7, 32, 33) -- required since every
    # comparison here mixes Scenarios 1-4 with 7/32straight/33straight.
...
def build_rehearsal_fit_data(base_model, source_pos, source_labels, target_pos, target_labels, rng):
    ...
    for seed_idx in range(N_SEEDS):
        ...
        for num_data_point in SWEEP_POINTS:
            ...
            X, y, sample_weight = build_rehearsal_fit_data(
                base_model, source_full_pos, source_full_labels,
...
def main():
    ...
    print(f"Running Stage 2 REHEARSAL baseline (batch 3) with model={args.model}", flush=True)
    ...
    print(f"\n=== Rehearsal baseline [{args.model}] batch-3 summary: mean top-2 accuracy per comparison ===", flush=True)
    ...
    # "_stage3" -- see module docstring: distinct from both
    # train_baselines_rehearsal.py's stage1_summary_{model}_rehearsal.json
    # (original Scenario-1-4 study) and
    # train_baselines_cross_scenario_rehearsal.py's
    # stage1_summary_{model}_rehearsal_stage2.json (the first 6-comparison
    # batch), so none of the three can overwrite another's summary.
