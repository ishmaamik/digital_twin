"""
Rehearsal-protocol Stage-2 study, batch 3b: k-NN, Random Forest, and their
Fourier-encoded variant, covering Scenario 33straight (College Ave, night)
as SOURCE against four targets:

    33straight -> 1, 33straight -> 2, 33straight -> 4, 33straight -> 32straight

Scope: only knn, rf, and fourier_knn (fourier_rf and TinyMLP are explicitly
out of scope for this batch, per instruction -- not tracked here at all).

Run as its own process, in parallel with (not instead of) whatever else is
already running -- in particular the ongoing MLP batch-3 run
(python/train_model_rehearsal_extended_batch3.py) and its sibling MLP
batch-4 run (python/train_model_rehearsal_extended_batch4.py), neither of
which this script touches or shares any output file with.

Kept as its own script/output files (own COMPARISONS, "_stage3b" summary
suffix) so nothing here can collide with batch 3's (or any earlier batch's)
results -- same rationale as batch 3's own docstring for why each batch gets
a distinct summary filename.
...
knn/fourier_knn) rows instead of just N. Across N_SEEDS=10 seeds, 4
comparisons, and 3 models this is cheap (minutes, not hours) but still
...
Must be run from the repository root, e.g.:
    python rehearsal_study/train_baselines_cross_scenario_rehearsal_stage3b.py --model knn
    python rehearsal_study/train_baselines_cross_scenario_rehearsal_stage3b.py --model rf
    python rehearsal_study/train_baselines_cross_scenario_rehearsal_stage3b.py --model fourier_knn
"""
import argparse
import json
import os
...
# The 4 comparisons this batch covers.
COMPARISONS = [
...
# fourier_rf and tinymlp are explicitly out of scope for this batch.
...
def main():
    ...
    print(f"Running Stage 2 REHEARSAL baseline (batch 3b) with model={args.model}", flush=True)
    ...
    print(f"\n=== Rehearsal baseline [{args.model}] batch-3b summary: mean top-2 accuracy per comparison ===", flush=True)
    ...
    # "_stage3b" -- distinct from batch 3's stage1_summary_{model}_rehearsal_stage3.json
    # and every earlier batch's summary filename, so none can overwrite another's.
    with open(os.path.join(RESULT_DIR, f"stage1_summary_{args.model}_rehearsal_stage3b.json"), "w") as f:
